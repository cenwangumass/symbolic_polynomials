        ...
        if rem == C::zero() {
            ...
            while i1 < result.powers.len() && i2 < rhs.powers.len() {
                match Ord::cmp(&result.powers[i1].0, &rhs.powers[i2].0) {
                    ...
                    Ordering::Equal => match Ord::cmp(&result.powers[i1].1, &rhs.powers[i2].1) {
                        Ordering::Less => return None,
                        Ordering::Equal => {
    ...
    P: Power,
{
    ...
    fn add(self, rhs: C) -> Self::Output {
        if rhs == C::zero() {
            ...
            Polynomial {
                monomials: vec![self.clone(), Monomial::from(rhs)],
            }
        }
    }
}
    ...
    P: Power,
{
    ...
    fn add(self, rhs: &'b Monomial<I, C, P>) -> Self::Output {
        if rhs.coefficient == C::zero() && self.coefficient == C::zero() {
            ...
            Polynomial {
                monomials: vec![self.clone(), rhs.clone()],
            }
        } else {
            Polynomial {
                monomials: vec![rhs.clone(), self.clone()],
            }
        }
    }
}
    ...
    P: Power,
{
    ...
    fn sub(self, rhs: &'b Monomial<I, C, P>) -> Self::Output {
        if self.coefficient == C::zero() && rhs.coefficient == C::zero() {
            ...
            Polynomial {
                monomials: vec![self.clone(), -rhs],
            }
        } else {
            Polynomial {
                monomials: vec![-rhs, self.clone()],
            }
        }
    }
}
